...
class blinnphong : public shader<vin, v2f>
{
public:
	blinnphong() {}
	blinnphong(const vin& vertin, const color& light_col, const color& surface_col, const double& gloss, const shared_ptr<tga> texture) : vertin(vertin), light_col(light_col), surface_col(surface_col), gloss(gloss), maintexture(texture) {}
	blinnphong(const vin& vertin, const color& light_col, const color& surface_col, const double& gloss, const shared_ptr<tga> maintexture, const shared_ptr<tga> spectexture) : vertin(vertin), light_col(light_col), surface_col(surface_col), gloss(gloss), maintexture(maintexture), spectexture(spectexture) {}
				...
				for(int i = xbegin; i < xend; i++)
				{
					for(int j = ybegin; j < yend; j++)
					{
						...
						if(dp->update(i, j, z))
						{
							...
							color diffuse_base_col = surface_col;
							//color specular_base_col = surface_col;
							double specular_rate = gloss;
							...
							if(!vertout.uvs.empty() && maintexture != nullptr)
							{
								...
								diffuse_base_col = color(int(tex_col_tga.bgra[2]) / 255., int(tex_col_tga.bgra[1]) / 255., int(tex_col_tga.bgra[0]) / 255., int(tex_col_tga.bgra[3]) / 255.);
								if(spectexture)
								{
									tga_color spec_col_tga = spectexture->get(int(uv.x() * (spectexture->get_width() - 1)), int((1 - uv.y()) * (spectexture->get_height() - 1)));
									specular_rate = int(spec_col_tga.bgra[0]) / 255.;
								}
							}

							// lambert's law
							color diffuse = light_col * diffuse_base_col * saturate(dot(worldnormal, -normalize(vertout.lightdir)));
							// blinn-phong specular
							vector view = normalize(vertout.camerapos - worldpos);
							vector half = normalize(view - vertout.lightdir);
							color specular = light_col * diffuse_base_col * pow(max(0, dot(worldnormal, half)), specular_rate) * 0.2;

							rf->drawpixel(i, j, diffuse + specular);
							//rf->drawpixel(i, j, specular);
						}
					}
				}
			}
