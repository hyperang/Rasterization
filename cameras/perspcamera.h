...
{
public:
	/* perspcamera public methods */
	perspcamera() {}
	perspcamera(point p, vector l, vector u, int ph, int pw, double n, double f, double h, double w) : height(h), width(w)
	{
		pos = p;
		...
		matrix4 mpersp2ortho = { vec4d(near, 0, 0, 0), vec4d(0, near, 0, 0), vec4d(0, 0, near + far, -near * far), vec4d(0, 0, 1, 0) };
		...
		world2clamp = matrixMulti({ tview, rview, mpersp2ortho, tortho, sortho, mviewport });
	}
